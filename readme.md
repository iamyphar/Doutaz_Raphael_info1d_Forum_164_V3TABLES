...
##### Cette démo. CRUD (Create Read Update Delete) presque complète sur les 3 tables de la base genre : Soit "t_genre"; "t_user" et la table intermédiaire "t_user_genre"
...
* Ouvrir "PyCharm". Cliquer sur le bouton à droite "GET FROM VCS"
    * Une fenêtre s'ouvre, copier le lien suivant dans le champ
      url : https://github.com/info164/164_OM_PYTHON_MYSQL_FLASK_EX_2_V1.git
    * Il est indispensable de créer un répertoire vide pour accueillir MON projet de référence. Cliquer sur l'icône
      répertoire
...
* Ouvrir "PyCharm", puis "File"-->"Open..." un projet et choisir le répertoire racine "
  VOTRENOM_VOTREPRENOM_VOTRECLASSE_VOTRESUJET_164_V3TABLES"
* Lire et réaliser les étapes du mode d'emploi de la démonstration
...
* Dans "PyCharm", cliquer avec le bouton droit de la souris sur le nom de la racine de votre projet et choisir "Compare
  With..." (CTRL-D", vous devez importer tout ce qui a changé dans la nouvelle version que je vous propose, mais ne pas
  écraser ce que vous avez déjà fait.
...
* Cliquer sur le menu déroulant de votre compte github (à droite en haut), choisir "your repositories"
    ...
    * Copier l'adresse de votre repository https://?????????/!!!!!
...
* Dans le menu "VCS", cliquer "Enable Version Control Integration", après cette action le nom "VCS" est remplacé par "
  Git"
...
* Vous devez définir la première fois le nom de votre repository distant coller votre adresse copiée avant : https:
  //?????????/!!!!!
  ????/VOTRENOM_VOTREPRENOM_VOTRECLASSE_VOTRESUJET_164_V3TABLES.git
* Il vous sera demandé de vous connecter au travers de "JetBrains, PyCharm", acceptez et remplir avec votre mot de passe
  de votre github
...
* Placer votre "DUMP" à la place de celui de ma "BD" genres. Votre "DUMP" doit se nommer "
  VOTRENOM_VOTREPRENOM_VOTRECLASSE_VOTRESUJET_164.SQL"
    * Dans le répertoire "database" vous devez placer vos files (PDF: Cahier des charges, MCD, MLD, dictionarys des
      données et SQL : requêtes, etc)
    * Ouvrir le file "database/1_ImportationDumpSql.py"
        * Cliquer avec le bouton droit sur l'onglet de ce file et choisir "run" (CTRL-MAJ-F10)
        * En cas d'erreurs : ouvrir le file ".env" à la racine du projet, contrôler les indications de connexion pour la
          bd.
* Test simple de la connexion à la BD
    * Ouvrir le file "database/2_test_connection_bd.py"
...
A) Modifier MES requêtes show (READ) du CRUD(Create Read Update Delete) sur la table "t_user"

* Ouvrir le file "users/gestion_users_crud.py"
* Modifier les requêtes "SELECT" sur la table "t_user" par des "SELECT" sur une de vos "t_????"
* Modifier dans le répertoire "TEMPLATES" la page html consacrée à l'affichage pour ma "t_user", il faut ouvrir "
  templates/users_show.html"
* Vous devez vous aider de "CTRL-R"(scope : file) ou "CTRL-SHIFT-R"(scope : tout le projet)
* utiliser la commande "magique" de "PyCharm" comparer deux files "CTRL-D" (menu "View"-->"Compare With...") entre le
  file de mon projet qui fonctionne et le votre qui est presque fonctionnel.
...
* Vous devez alors changer les noms de variables en Python pour les adapter à votre sujet
    * Exemple : "strsql_users_show" doit s'écrire avec un nom qui correspond mieux à votre sujet de BD

C) Faites souvent des "COMMITS" et des "PUSH" sur votre "GITHUB" (
VOTRENOM_VOTREPRENOM_VOTRECLASSE_VOTRESUJET_164_V3TABLES)

D) Recommencer les étapes A) à C) pour l'action "CREATE" pour la table "t_user"

E) Recommencer les étapes A) à C) pour l'action "UPDATE" pour la table "t_user"

F) Recommencer les étapes A) à C) pour l'action "DELETE" pour la table "t_user"

G) Passez à une autre table (MAIS PAS UNE TABLE INTERMÉDIAIRE) de votre BD

# Évaluation de votre module 164

Dans le but de l'évaluation du module 164, chaque semaine, vous devez déposer votre travail (file zip du repository de
votre Github) sur le moodle de l'EPSIC
...
* Je vais observer votre avance sur votre github où vous avez défini mon adresse couriel officielle comme "
  collaborateur" à votre github
...
* Dans votre github. Créer un file "zip" à télécharger sur votre disque local vous DEVEZ le déposer sur Moodle de
  l'EPSIC dans la semaine correspondante
* Le lien de votre github doit être dans un file "txt" que vous DEVEZ déposer sur Moodle de l'EPSIC dans la semaine
  correspondante
* Il faut respecter mes consignes énoncées dans ce file "readme.md" et sur le MOODLE de l'EPSIC. Sinon, vous ne pourrez
  plus prétendre à l'excellence, soit la note de "6"
    * Le but de ce module 164 est de faire un CRUD sur votre BD, pas de faire du code. Donc faites ce qui est demandé
      pas plus, ni moins
